...
static NSString *const JDRouteParamPattern = @":[a-zA-Z0-9-_][^/]+";
static NSString *const JDRouteParamNamePattern = @":[a-zA-Z0-9-_]+";
static NSString *const JDRouteParamMatchPattern = @"([^/]+)";

@interface JDRegularExpression ()
@property (nonatomic, strong) NSArray *routerParamNamesArr;
@end

@implementation JDRegularExpression

+ (JDRegularExpression *)expressionWithPattern:(NSString *)pattern{
    NSError *error;
    JDRegularExpression *exp = [[JDRegularExpression alloc] initWithPattern:pattern options:(NSRegularExpressionCaseInsensitive) error:&error];
    if (error) {
        NSLog(@"JDRegularExpression ===== %@", error);
    }
    return exp;
}
...
- (instancetype)initWithPattern:(NSString *)pattern options:(NSRegularExpressionOptions)options error:(NSError * _Nullable __autoreleasing *)error{
    ...
    NSString *transformedPattern = [JDRegularExpression transfromFromPattern:pattern];
...
-(JDMatchResult *)matchResultForString:(NSString *)string{
    ...
    JDMatchResult * result = [[JDMatchResult alloc]init];
    if (array.count == 0) {
        return result;
    }
    result.match = YES;
...
+(NSString*)transfromFromPattern:(NSString *)pattern{
    ...
    NSError * err;
    NSRegularExpression * paramNamePatternEx = [NSRegularExpression regularExpressionWithPattern:JDRouteParamNamePattern options:NSRegularExpressionCaseInsensitive error:&err];
    /*
    ...
    for (NSString * paramPatternString in paramPatternStrings) {
        ...
        if (replaceParamPatternString.length == 0) {
            replaceParamPatternString = JDRouteParamMatchPattern;
        }
...
+(NSArray<NSString *> * )paramPatternStringsFromPattern:(NSString *)pattern{
    NSError *err;
    NSRegularExpression * paramPatternEx = [NSRegularExpression regularExpressionWithPattern:JDRouteParamPattern options:NSRegularExpressionCaseInsensitive error:&err];
...
+(NSArray *)routeParamNamesFromPattern:(NSString *)pattern{
    NSRegularExpression *paramNameEx = [NSRegularExpression regularExpressionWithPattern:JDRouteParamNamePattern options:NSRegularExpressionCaseInsensitive error:nil];
